...
(function ($) {
    ...
    function woosg_active_settings() {
        if ($('#product-type').val() == 'woosg') {
            ...
            $('._tax_status_field').closest('.options_group').addClass('show_if_woosg');
            $('#_downloadable').closest('label').addClass('show_if_woosg').removeClass('show_if_simple');
            $('#_virtual').closest('label').addClass('show_if_woosg').removeClass('show_if_simple');
            ...
            $('._tax_status_field').closest('.options_group').removeClass('show_if_woosg');
            $('#_downloadable').closest('label').removeClass('show_if_woosg').addClass('show_if_simple');
            $('#_virtual').closest('label').removeClass('show_if_woosg').addClass('show_if_simple');
    ...
    function woosg_format_money(number, places, symbol, thousand, decimal) {
        ...
        var negative = number < 0 ? '-' : '',
            i = parseInt(number = woosg_round(Math.abs(+number || 0), places).toFixed(places), 10) + '', j = 0;
        if (i.length > 3) {
            j = i.length % 3;
        }
        return symbol + negative + (j ? i.substr(0, j) + thousand : '') +
            i.substr(j).replace(/(\d{3})(?=\d)/g, '$1' + thousand) +
            (places ? decimal + woosg_round(Math.abs(number - i), places).toFixed(places).slice(2) : '');
    }
